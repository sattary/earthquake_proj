...
\author{\textbf{Research Proposal (Version 3.0)}}
\date{\today}
...
\begin{equation}
    ...
    \label{eq:poisson}
\end{equation}
The integral is approximated by Monte Carlo summation over the physics collocation points. Events below the magnitude of completeness $M_c$ (estimated via the maximum curvature method \cite{Wiemer2000}) are excluded.
...
\begin{itemize}
    ...
    \item \textbf{Non-dimensionalization}: all quantities are scaled by characteristic values ($S_0 = 1$~GPa, $V_0 = 10^{-9}$~m/s, $L_0 = 10^6$~m) to ensure $O(1)$ loss residuals.
\end{itemize}
