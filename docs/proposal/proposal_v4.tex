...
\end{enumerate}
The resulting unified magnitude is written as a single column $M_w^{\text{uni}}$ in the cleaned catalog. This rule is applied \emph{before} the magnitude-of-completeness filter and before declustering (the latter two steps will be added in upcoming amendments P.5 and P.6; for now we note that magnitude unification is the first preprocessing step in the pipeline).
