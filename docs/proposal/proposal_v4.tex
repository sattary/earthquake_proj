...
%   P.3  UTM zone handling: single-zone (zone 39) restricted to Central
%        Iran subset (48E-54E, 25N-40N) with conformal distortion analysis;
%        also fixes GPS count to N=209 in Data Sources item 1.
%   P.4  Magnitude unification rule: Mw priority, else Scordilis 2006
%        conversions (new Section 2.5.4 + Scordilis2006 bibitem).
%   P.5  Magnitude of completeness M_c: MAXC + 0.2 and GFT (90%),
%        adopted M_c = max(both). New Section 2.5.5.
%   P.6  Aftershock declustering: Zaliapin-Ben-Zion 2008 nearest-neighbor
%        with b=1.0, d_f=1.6, eta_0=1.0. New Section 2.5.6.
%   P.7  Constitutive law: viscous-only. New Section 2.7 'Constitutive
%        Law Scope' notes elastic mode is deprecated and out of scope
%        for the interseismic timescale; code cleanup in same commit.
%
% Pending amendments (queued for review):
%   P.8  Network hyperparameter formalization
%   P.10 Random seed policy
% (P.9 superseded by P.3's data count fix.)
% ============================================================================
...
\subsubsection*{UTM zone handling.}
The Iranian plateau spans UTM zones 38--41 (longitudes 42$^\circ$E to 66$^\circ$E). The transverse Mercator projection used in UTM introduces a conformal scale error of order $(s/R)\tan\varphi \cdot \Delta\lambda^2$ at distance $\Delta\lambda$ from the central meridian, where $s$ is the along-meridian distance and $R$ is Earth radius. At the zone edge ($|\Delta\lambda| = 3^\circ$) this error reaches $\sim 1$ part in 800, which is well below the GPS azimuth noise floor of $\sim 2^\circ$ and therefore acceptable for the present inversion.

For the present study we adopt a \textbf{single-zone} (zone 39, central meridian 51$^\circ$E) projection restricted to the Central Iran subset of the GPS, catalog, and tomography data (longitudes 48$^\circ$E--54$^\circ$E). This restriction is conservative: it keeps the conformal distortion under $\sim 0.4$ parts in 800 across the domain, and it is consistent with the spatial coverage of the Morteza et al.\ (2023) P-wave tomography model. A multi-zone analysis with continuous re-projection at zone boundaries is deferred to follow-up work; the framework's coordinate transformer accepts a configurable zone number so that switching to multi-zone is a runtime parameter change rather than a code refactor.
...
\subsubsection{Catalog Magnitude Unification}
\label{sec:mag_unif}
Historical and instrumental catalogs typically report events with multiple magnitude scales simultaneously ($M_w$, $M_s$, $m_b$, $M_L$, $M_I$). Different scales saturate at different true moment magnitudes: $m_b$ saturates near $6.5$, $M_s$ near $8.0$, while $M_w$ is unbounded. Taking the maximum across reported scales (a heuristic used in earlier versions of the code) systematically \textit{overestimates} the true moment magnitude, because a high $m_b$ value (which reflects high-frequency radiation, not size) is taken to indicate a larger earthquake than it physically represents.

We therefore adopt the following \textbf{unification rule}, applied per event in priority order:
\begin{enumerate}
    \item If $M_w$ is reported, use it directly.
    \item Else, if $M_s$ is reported and $3.0 \le M_s \le 6.1$, convert via \cite{Scordilis2006}:
          \begin{equation}
              M_w = 0.67\,M_s + 2.07.
              \label{eq:ms_to_mw}
          \end{equation}
          For $M_s > 6.1$ the $M_s = M_w$ identity is used (the $M_s$ scale is already calibrated to moment in this range).
    \item Else, if $m_b$ is reported and $m_b \le 6.1$, convert via \cite{Scordilis2006}:
          \begin{equation}
              M_w = 0.85\,m_b + 1.03.
              \label{eq:mb_to_mw}
          \end{equation}
          For $m_b > 6.1$ the conversion is unreliable and the event is dropped.
    \item Else (only $M_L$, $M_I$, or no magnitude reported), the event is dropped.
\end{enumerate}
The resulting unified magnitude is written as a single column $M_w^{\text{uni}}$ in the cleaned catalog. This rule is applied \emph{before} the magnitude-of-completeness filter (Section~\ref{sec:mc_est}) and before declustering (Section~\ref{sec:decluster}); it is the first preprocessing step in the pipeline.

\subsubsection{Magnitude of Completeness}
\label{sec:mc_est}
The magnitude of completeness $M_c$ is the minimum magnitude above which the declustered catalog is assumed to record all events. Below $M_c$ the catalog is incomplete due to detection thresholds, network geometry, or human reporting bias. Because $\mathcal{L}_{\text{seis}}$ (Eq.~\ref{eq:poisson_mc}) uses $R(\mathbf{x}_i)$ evaluated only at events with $M \ge M_c$, an underestimate of $M_c$ inflates the catalog with undetected events and biases the rate estimate; an overestimate of $M_c$ discards useful data.

We estimate $M_c$ from the declustered, magnitude-unified catalog using two cross-checked methods \cite{Wiemer2000}:
\begin{enumerate}
    \item \textbf{Maximum Curvature (MAXC).} $M_c^{\text{MAXC}}$ is the modal magnitude of the non-cumulative Gutenberg-Richter frequency-magnitude distribution, defined as the magnitude bin at which the count $N(M)$ peaks. MAXC is known to underestimate the true $M_c$ by approximately $0.2$ units when the catalog rolls off rapidly, so the corrected estimate is $M_c^{\text{MAXC, corr}} = M_c^{\text{MAXC}} + 0.2$.
    \item \textbf{Goodness-of-Fit Test (GFT).} $M_c^{\text{GFT}}$ is the lowest magnitude above which the observed frequency-magnitude distribution matches the synthetic Gutenberg-Richter power law at the 90\% confidence level, where the residual is computed as $R(M) = 100 - \bigl(N_{\text{obs}}(M \ge M) / N_{\text{exp}}(M \ge M)\bigr) \cdot 100$, with the 90\% threshold being $R \le 90\%$.
\end{enumerate}
The adopted $M_c$ is
\begin{equation}
    M_c = \max\bigl(M_c^{\text{MAXC, corr}},\; M_c^{\text{GFT}}\bigr).
    \label{eq:mc_def}
\end{equation}
The max-combination prevents the GFT estimate from undershooting when it identifies a higher threshold than MAXC, while the MAXC $+0.2$ correction prevents the typical MAXC bias from propagating. Both intermediate values and the adopted $M_c$ are reported alongside the number of events retained, $N_{\text{eq}}$, so that the sensitivity of subsequent results to the choice of $M_c$ is auditable. Events with $M_w^{\text{uni}} < M_c$ are dropped before the declustering step.

\subsubsection{Aftershock Declustering}
\label{sec:decluster}
The Poisson NLL of Eq.~\ref{eq:poisson_mc} assumes that the catalog is a stationary, independent point process in space. Real seismicity catalogs violate this: aftershock sequences are triggered cascades, not independent events, and they cluster strongly in space and time. Including triggered events as if they were independent would (i) inflate the apparent rate $R(\mathbf{x})$ in active aftershock zones and (ii) bias the rate-vs-CFF relationship.

We therefore decluster the catalog using the Zaliapin--Ben-Zion nearest-neighbor method \cite{Zaliapin2008}. For each event $j$ we identify the most likely parent $i$ (the event with the smallest rescaled distance $\eta_{ij}$ among those that occurred before $j$) and classify $j$ as a triggered offspring if $\eta_{ij} < \eta_0$. The rescaled distance is
\begin{equation}
    \eta_{ij} = T_{ij} \cdot R_{ij}^{d_f} \cdot 10^{-b\,M_i / 2},
    \label{eq:zb_distance}
\end{equation}
where $T_{ij}$ is the temporal separation in years, $R_{ij}$ is the 3D hypocentral separation in km, $d_f$ is the fractal dimension of the earthquake spatial distribution, $b$ is the regional Gutenberg-Richter $b$-value, and $M_i$ is the parent moment magnitude. The units are absorbed by the empirical constants in \cite{Zaliapin2008} (originally $T$ in years, $R$ in km, $b$ in moment-magnitude units).

We adopt the following \textbf{declustering parameters}, consistent with the crustal-tectonic setting of Central Iran:
\begin{itemize}
    \item $b = 1.0$ (regional Gutenberg-Richter $b$-value, taken as the median of $b$-values reported for Central Iran; sensitivity to $b \in [0.8, 1.2]$ is reported in the ablation tables).
    \item $d_f = 1.6$ (fractal dimension of the spatial earthquake distribution; standard value for epicentral distances in tectonically active crust \cite{Zaliapin2008}).
    \item $\eta_0 = 1.0$ (the threshold below which an event is classified as triggered; the standard choice for crustal seismicity that gives a declustered catalog with $b$-value approximately equal to the regional $b$-value when the $b$-value input is correct).
\end{itemize}
The result of declustering is a catalog in which each event is flagged as either \texttt{independent} (a potential parent) or \texttt{triggered} (an offspring). Only \texttt{independent} events are passed to the Poisson NLL term. The number of triggered events removed, $N_{\text{trig}} = N_{\text{pre}} - N_{\text{post}}$, is reported alongside $N_{\text{eq}}$ and $M_c$ in the validation tables. The declustering step is applied \emph{after} magnitude unification (Section~\ref{sec:mag_unif}) and \emph{after} the magnitude-of-completeness filter (Section~\ref{sec:mc_est}); it is the third and final preprocessing step before the catalog is consumed by $\mathcal{L}_{\text{seis}}$.
...
\subsection{Constitutive Law Scope}
The framework supports only the viscous (Stokes) constitutive law. An elastic (Hooke) mode was implemented in earlier versions of the code as a prototype but is not part of the present study. The Stokes approximation is justified on the interseismic timescale of $\sim 10^4$--$10^5$~yr (Section~\ref{sec:coords}); the elastic response is relevant only on the co-seismic timescale ($\sim$ s), which is outside the scope of the inversion. Future work may add a co-seismic elastic module to the same network, gated by a switch on the strain rate.
...
\begin{enumerate}
    \item \textbf{GPS strain-rate azimuths}: $N = 209$ observations from the combined Khorrami et al.\ \cite{Khorrami2019} and Rayisi \cite{Rayisi2016} datasets, restricted to the Central Iran subset (longitudes 48$^\circ$E--54$^\circ$E, latitudes 25$^\circ$N--40$^\circ$N) consistent with the UTM zone 39 choice in Section~\ref{sec:coords}. The azimuths provide maximum compressive strain-rate orientation. The full combined GPS+interferometric catalog available to the broader project (estimated $\sim 2000$ observations across the whole plateau) is left for follow-up work after the single-zone analysis is validated.
...
\bibitem{Scordilis2006}
Scordilis, E.M. (2006). Empirical global relations converting $M_S$ and $m_b$ to moment magnitude. \textit{J.\ Seismol.}\ \textbf{10}, 225--236.
