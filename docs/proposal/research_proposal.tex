...
\begin{itemize}
    \item \textbf{Zhang et al. (2022)}: Used PINNs for stress inversion but assumed purely elastic regimes and relied on dense synthetic data.
    \item \textbf{ViscoelasticNet (2023)}: Applied viscoelastic PINNs to non-Newtonian fluids, lacking the background stress complexities of tectonics.
    \item \textbf{Degen et al. (2023)}: Modeled general geodynamics with synthetic global models.
\end{itemize}
\textbf{No study has yet applied Viscoelastic PINNs to invert for the 3D tectonic stress tensor} using only sparse \textbf{orientation constraints} (GPS azimuths) in a complex continental collision zone. This solves the ``inverse problem'' without the need for dense ground truth magnitude data.
...
\begin{enumerate}
    \item \textbf{Andersonian Consistency}: The inverted stress tensor must match dominant faulting styles (e.g., vertical $\sigma_3$ in Zagros reverse faulting zones).
    \item \textbf{Borehole Breakout (BBO) Comparison}: We will validate against \textit{in-situ} stress orientations from the \textbf{World Stress Map (WSM 2025)}. A successful model must achieve an angular misfit of $< \textbf{15$^\circ$-20$^\circ$}$.
    \item \textbf{Seismicity Hindcasting}: Comparing log-likelihood scores against \textbf{Coulomb Stress Transfer (CST)} and \textbf{XGBoost} baselines for historical events (2006-2023).
\end{enumerate}
